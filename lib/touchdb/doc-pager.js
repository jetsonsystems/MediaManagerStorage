    ...
    var p = dIt.next().then(
      ...
      function(pageRows) {
        if (!pageRows || (pageRows.length === 0)) {
          ...
          return rowsToPage(pageRows, 0, pageSize);
        }
      },
      function(err) {
    ...
    var p = dIt.next().then(
      ...
      function(pageRows) {
        if (pageRows && (pageRows.length > 1)) {
          ...
          return rowsToPage(reversed, start, pageSize);
        }
        else {
          //
...
function isCursor(cursor) {
  return (_.isObject(cursor) && _.has(cursor, 'key') && _.has(cursor, 'id') && _.isObject(cursor.key) && _.isString(cursor.id));
}
