    ...
    var p = dIt.next().then(
      ...
      function(pageRows) {
        if (pageRows && (pageRows.length > 1)) {
          ...
          var start;
          var lastRow = reversed[reversed.length-1];

          if (cursor.id !== lastRow.id) {
            start = (reversed.length > pageSize) ? reversed.length - pageSize: 0;
          }
          else {
            start = (reversed.length > (pageSize+1)) ? reversed.length - pageSize - 1: 0;
          }
