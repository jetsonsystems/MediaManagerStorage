//
//  MediaManagerStorage/lib/touchdb: Interface to TouchDB.
//
...
//
//    For additional documentation see MediaManagerStorage/lib/touchdb/README.md
//
...
var docsModule = require('./docs.js');
var DocIteratorModule = require('./doc-iter');
var DocPagerModule = require('./doc-pager');
var ViewHelpersModule = require('./view-helpers');

var moduleName = './lib/touchdb';
...
module.exports = function touchdbModule(config, opts) {
  ...
  var docs = docsModule(config);

  var docPagerModuleInst = DocPagerModule(config.database, config.local.host, config.local.port);

  var viewHelpersModuleInst = ViewHelpersModule(config.database, config.local.host, config.local.port);
  ...
  var newInst = Object.create({}, { 
    ...
    // CRUD:
    //
    docFactory: { value: function(className, attr, callback) {
      docs.docFactory(className, attr, callback);
    }},
    read: { value: function(id, callback) {
      docs.read(id, callback);
    }},
    update: { value: function(id, callback) {
      docs.update(id, callback);
    }},
    //
    // Document iteration:
    //
    DocIterator: { value: DocIteratorModule(config.database, config.local.host, config.local.port ).Iter },
    DocPager: { value: docPagerModuleInst.Pager },
    DocPagerStopIteration: { value: docPagerModuleInst.StopIteration },
    DocPagerIterationError: { value: docPagerModuleInst.IterationError },
    isCursor: { value: docPagerModuleInst.isCursor },
    //
    // View helpers:
    //
    iterateOverView: { value: viewHelpersModuleInst.iterateOverView },
    iterateOverViewKeys: { value: viewHelpersModuleInst.iterateOverViewKeys },
    //
    // Sync:
    //
    sync: { value: sync },
