...
//    view
//    options
//      mapOnlyView: if specified and true, reduce=false is NOT passed down.
//        Note, in CouchDB version 1.0.2, reduce=false is NOT an error on map only views.
//        In older versions of couchdb it is an error.
//      startKey: key to start paging at.
//      startKeyDocId: doc id associated with start key.
//      endKey: key to stop paging at.
...
//      skip: Page by skipping as opposed to updating startkey. Default is false. 
//        Note, use this when a view can return multiple docs for the same key. In TouchDB
//        startkey_docid appears to NOT work.
//
function DocIterator(pageSize, designDoc, view, options) {
  ...
  var endKey = options.endKey || undefined;
  var endKeyDocId = options.endKeyDocId || undefined;
  ...
  var fetchFromDB = function(n, callback) {
    ...
    if (!_.has(options, mapOnlyView) || (options.mapOnlyView === false)) {
      view_opts.reduce = false;
    }
    if (startKey) {
    ...
    if (endKey) {
      view_opts.endkey = endKey;
      if (endKeyDocId) {
        view_opts.endkey_docid = endKeyDocId;
      }
    }
    if (n) {
      view_opts.limit = n;
  ...
  var transformAndFilter = function(rows, numToTransform) {
    ...
    if (options.transform) {
      ...
      log.debug(lp + 'No transform required, returning raw rows, filtering...');
