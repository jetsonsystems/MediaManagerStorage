  ...
  var Uploader = function(accountDesc, persistDir, options) {
    ...
    function doUpload(toUpload, callback) {
      if (refreshingAccessToken === true) {
        ...
        callback(cbError, toUpload.asset);
        return;
      }
      ...
      client.upload(assetDesc,
                    {
                      ...
                      callback: function(error, response, body) {
                        var cbErr = undefined;
                        if (error) {
                          logger.info(logPrefix + 'Upload error, error - ' + error);
                          ...
                          logger.info(logPrefix + 'status code - ' + response.statusCode + ', response body - ' + util.inspect(body));
                        }
                        if (options.callback) {
                          options.callback(cbErr, assetDesc);
                        }
                      }});
    }
  };
