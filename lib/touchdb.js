...
var synchronizerFactory = function(config, options, callback) {
  ...
  log.info('touchdb.js: About to create synchronizer...');
  ...
  log.info('touchdb.js: Creating synchronizer w/ id - %s', synchronizerId);
  ...
  var monitor = function(replication, options, callback) {
    log.info('touchdb.js.monitor: Monitoring replication, type - %s', replication.type);
    ...
    var check = function(url, callback) {
      request(url, function(error, response, body) {
        ...
        log.info('touchdb.js.monitor: Monitored replication, type - %s, check number %s, url - %s, body type - %s, body - %j', 
        ...
        if (error) {
          ...
          log.error('touchdb.js.monitor: Replication error - %s', error);
        }
          ...
          log.info('touchdb.js.monitor: Replication state - %s', replication.state);
        }
        else {
          log.info('touchdb.js.monitor: Response status - %s', response.statusCode);
        }
        callback();
      });
    };
    ...
    var checked = function() {
      if (((maxChecks === undefined) || (numChecks < maxChecks)) && (replication.state === 'triggered')) {
        ...
        log.info('touchdb.js.monitor: Scheduling next check, numChecks - %s, maxChecks - %s, check interval - %s', 
    ...
    log.info('touchdb.js.monitor: About to perform checks, numChecks - %s, maxChecks - %s, check interval - %s', 
				numChecks, maxChecks, checkInterval);
  ...
  var replicate = function(replication, callback) {
    ...
    log.info('touchdb.js: About to replicate, type - %s, url - %s, source - %s, target - %s',
      ...
      body: { "_id": replication.id,
              ...
              "create_target": true }
                 ...
                 function(error, response, body) {
                   if (error) {
                     log.info('touchdb.js: Replication error, type - %s, error - %s', replication.type, error.message);
                   }
                   else {
                     log.info('touchdb.js: Replication response, type - %s, status code - %s, body - %j', 
                   ...
                   if (!error && response.statusCode === 201) {
                     ...
                     synchronizer.state = 'error';
                     log.error('touchdb.js: Error triggering replication...');
                     synchronizer.emit(replication.events.completed);
                     callback(error);
                   }
                 });
    synchronizer.state = 'triggered';
    log.info('touchdb.js: Replication triggered...');
  ...
  if (options && _.has(options, 'passive') && options.passive) {
    ...
    log.info('touchdb.js: About to monitor passive synchronization...');
    async.waterfall([
      function(next) {
    ...
    log.info('touchdb.js: Annotating non-passive synchronizer with run function...');
    synchronizer.run = function() {
      synchronizer.state = 'triggered';
      log.info('touchdb.js: Synchronization triggered...');
      synchronizer.emit('sync.started', synchronizer);
      async.waterfall([
        function(next) {
          log.info('touchdb.js: Triggering push...');
          replicate(synchronizer.push, next);
        },
        function(next) {
          log.info('touchdb.js: Triggering pull...');
          replicate(synchronizer.pull, next);
        }
      ], function(err) {
        if (err) {
          log.error('touchdb.js: Error during synchronization, error - %s', err);
        }
        synchronizer.state = 'completed';
        synchronizer.emit('sync.completed', synchronizer);
      });
    };
    log.info('touchdb.js: Annotation completed...');
  }
...
var changesFeedFactory = function(config, opts) {
  log.info('touchdb.js: Creating changes feed...');
  ...
  var filterChange = function(change) {
    ...
    if (_.has(change, 'doc')) {
      ...
      log.info('Processing change doc: id - %s, doc - %j', change.id, doc);
  ...
  var processChanges = function(buffer) {
    var that = this;
    log.info('Processing changes w/ config - %j', that.config);
        ...
        if (part.length) {
          try {
            ...
            if (_.has(change, 'seq') && _.has(change, 'id')) {
              log.info('Processing change: seq - %s, id - %s', change.seq, change.id);

              var changeEvent = filterChange.call(that, change);
              if (changeEvent) {
                log.info('About to emit change event, type - %s', changeEvent.type);
            ...
            log.error('Error processing change, data - %s, error - %s', part, err);
          }
        }
      });
      return _.last(parts);
...
module.exports = function touchdbModule(config, opts) {
  ...
  var info = function(callback) {
    ...
    log.info('touchdb.info: About to request DB info, url - %s', options.url);
    //
  ...
  log.info('touchdb.js: Creating touchdb instance w/ config of - %j', config);
